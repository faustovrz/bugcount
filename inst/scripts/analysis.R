...
# quartz()
plot_count.fit(wf.count.fit, main = "Nymphs")
...
# quartz()
plot_fit.nb.glm(wf.count, by.experiment)


# quartz()
plot_fit.nb.glm(wf.count,by.experiment,type="density")
...
# quartz()
  ...
  plot_wide.cor(wf.by.exp, main = paste(cross,cor.title))
...
# quartz()
plot_check.cor(check.by.exp, main = paste("Checks", cor.title))
...
# quartz()
plot_fit.nb.glm(wf.check,fit)

# quartz()
plot_fit.nb.glm(wf.check,fit,type = "density")
...
# quartz()
plot_gg.infestationXclone(posthoc)


# quartz()
plot_infestationXclone(wf.check)
...
exp.grp <-list(CM8996 = list(1,2,4,5,c(1,2),c(2,4),c(4,5),c(1,2,4), c(2,4,5)),
               GM8586 = list(3,6,7, c(3,6), c(3,7), c(3,6,7)))
...
for (cross in levels(wf.count$exp.cross)){
  ...
  # # quartz()
  # plot_count.fit(by.cross.count.fit, 
  ...
  colnames(posthoc)
# quartz()
  plot_cross.anova(posthoc,exp.allowed)
  ...
  # Plot post hoc
# quartz()
  plot_cross.nb.fit(posthoc,exp.allowed)
...
# quartz()
plot_ef.density(ef,"geometric.mean.eff","experiment")
...
# quartz()
plot_ef.cor(ef.gmean,main = paste(cross,"/",control,cor.title))
